...
function UnitsAnalysis() {
  ...
  useEffect(() => {
    ...
    // Filter based on primary selections
    ...
    // Update primary filter options
    setAvailableLocations(getUniqueValues(filteredByOffering, 'Location'));
    setAvailableAssetTypes(getUniqueValues(
      selectedLocations.length > 0 
        ? filteredByOffering.filter(row => selectedLocations.includes(row.Location))
        : filteredByOffering,
      'Asset Type'
    ));
    setAvailableUnitTypes(getUniqueValues(filtered, 'Unit Type'));

    // Update secondary filter options
    setAvailableFinishing(getUniqueValues(filtered, 'Finishing Specs.'));
    setAvailableDevelopers(getUniqueValues(filtered, 'Developer Name'));
    setAvailableProjects(getUniqueValues(filtered, 'Project Name '));
    setAvailablePayments(getUniqueValues(filtered, 'Payment Yrs'));
        ...
        <div className="toggle-container">
          ...
          <div
            ...
            onClick={() => {
                ...
                handleReset(); // ✅ clear all filters when switching
              }}
